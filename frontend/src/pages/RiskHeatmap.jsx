...
export default function RiskHeatmap() {
  ...
  useEffect(() => {
    async function fetchData() {
      try {
        setLoading(true);
        const result = await getRiskHeatmap();
        setData(result);
  ...
  const getRiskColor = (score) => {
    if (score >= 100) return '#ff007f'; // Critical (rose)
    if (score >= 50) return '#ffaa00'; // High (amber)
    if (score >= 20) return '#00bbf9';  // Moderate (cyan)
    ...
    return (
      <div className="flex-1 flex items-center justify-center p-12">
        <div className="flex flex-col items-center gap-3">
          ...
          <span className="text-xs text-text-muted">Aggregating complexity, churn, and security metrics...</span>
        </div>
      </div>
    );
  }
  ...
  return (
    <>
      <PageHeader 
        title="Composite Risk Heatmap" 
        subtitle="Hotspots calculated from complexity, churn, and security vulnerabilities" 
      />
        ...
        <div className="flex-1 flex flex-col gap-6 min-w-0">
          <div className="card-base flex-1 flex flex-col min-h-[450px]">
            <div className="border-b border-border-subtle/50 pb-3 mb-4 flex items-center justify-between">
              <div>
                <h3 className="font-bold text-xs uppercase text-text-muted tracking-wider">Overall Risk Matrix</h3>
                <p className="text-[10px] text-text-muted mt-0.5">Colors indicate severity of composite risk score.</p>
              </div>
            </div>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 xl:grid-cols-5 gap-3 overflow-y-auto max-h-[600px] pr-1">
              {files.map((file, i) => {
                ...
                const scoreColor = getRiskColor(file.total_risk);
                return (
                  <div
                    key={i}
                  ...
                  >
                    ...
                    <div className="flex justify-between items-end mt-2 pl-1">
                      <span className="text-[9px] uppercase font-bold tracking-widest text-text-muted">Total Risk</span>
                      <span className="font-extrabold text-[13px] text-on-surface">
                        {file.total_risk.toFixed(1)}
                      </span>
                    </div>
                  </div>
                );
        ...
        <div className="w-full lg:w-[400px] shrink-0">
          <div className="card-base sticky top-24 flex flex-col gap-5">
            <div className="border-b border-border-subtle/50 pb-3 flex items-center gap-2">
              ...
              <div>
                <h3 className="font-bold text-xs uppercase tracking-wider text-on-surface">Risk Detail</h3>
                <p className="text-[10px] text-text-muted">Breakdown of composite score</p>
              </div>
            </div>
              ...
              <div className="flex flex-col gap-4 text-xs">
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div className="p-3 bg-surface-container-low/40 border border-border-subtle rounded-lg text-center">
                    <span className="text-[9px] text-text-muted uppercase tracking-widest block mb-0.5">Complexity Risk</span>
                    <span className="text-lg font-extrabold text-signal-amber">{selectedFile.complexity_score}</span>
                  </div>
                  <div className="p-3 bg-surface-container-low/40 border border-border-subtle rounded-lg text-center">
                    <span className="text-[9px] text-text-muted uppercase tracking-widest block mb-0.5">Churn Risk</span>
                    <span className="text-lg font-extrabold text-signal-cyan">{selectedFile.churn_score}</span>
                  </div>
                </div>
                
                <div className="p-3 bg-surface-container-low/40 border border-border-subtle rounded-lg text-center">
                  <span className="text-[9px] text-text-muted uppercase tracking-widest block mb-0.5">Security Score</span>
                  <span className="text-lg font-extrabold text-signal-rose">{selectedFile.security_score}</span>
                </div>
                ...
                <div className="p-3 bg-[#12121e] border border-border-subtle rounded-lg">
                  <div className="flex justify-between font-semibold mb-1.5">
                    ...
                    <span className="font-bold text-primary">{selectedFile.total_risk.toFixed(1)}</span>
                  </div>
                  <div className="h-1.5 bg-surface-container-low rounded-full overflow-hidden">
                    <div 
                      className="h-full bg-primary" 
                      style={{ width: `${Math.min(100, (selectedFile.total_risk / 150) * 100)}%`, backgroundColor: getRiskColor(selectedFile.total_risk) }} 
                    />
                  </div>
                </div>
              </div>
